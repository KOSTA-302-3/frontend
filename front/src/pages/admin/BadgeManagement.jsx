...
const BadgeManagement = () => {
  ...
  const filteredBadges = badges.filter((badge) => {
    ...
    return badge.name?.toLowerCase().includes(term) || badge.description?.toLowerCase().includes(term);
  });
  ...
  return (
    <Container>
      ...
      {loading ? (
        ...
        <EmptyMessage>{searchTerm ? "검색 결과가 없습니다." : "등록된 뱃지가 없습니다."}</EmptyMessage>
      ) : (
        <BadgeGrid>
          {filteredBadges.map((badge, index) => (
            <BadgeCard key={badge.badgeId || `badge-${index}`}>
              ...
              <BadgeInfo>
                ...
                <BadgePrice>{badge.price ? badge.price.toLocaleString() : "0"} 포인트</BadgePrice>
              </BadgeInfo>
              <BadgeActions>
                <ActionButton type="edit" onClick={() => handleOpenModal(badge)}>
                  <EditOutlined /> 수정
                </ActionButton>
                <ActionButton type="delete" onClick={() => handleDelete(badge.badgeId)}>
                  <DeleteOutlined /> 삭제
                </ActionButton>
              </BadgeActions>
            </BadgeCard>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 24 }}>
          ...
          >
            <Input.TextArea rows={3} placeholder="뱃지에 대한 설명을 입력하세요" />
          </Form.Item>
          ...
          <Form.Item label="뱃지 이미지" name="badgeImage">
            <Upload {...uploadProps}>
              {fileList.length === 0 && (
                <div>
                  <PlusOutlined />
